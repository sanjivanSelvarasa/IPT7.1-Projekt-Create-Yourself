...
<template>
  <div
    class="z-999 fixed top-0 left-0 w-full bg-[var(--surface-color)] shadow-sm h-[65px] flex items-center justify-center"
  >
    <div class="flex items-center justify-between w-[1200px] px-4 py-2 h-full">
      ...
      <div class="md:flex hidden h-full items-center">
        <ul
          class="flex items-center justify-center gap-4 text-sm text-[var(--text-color-light)] font-semibold"
        >
      ...
      <div class="flex items-center justify-center gap-4">
        <RouterLink
          to="/login"
          class="hover:border-[var(--primary-color)] hover:text-[var(--primary-color)] duration-150 transition px-4 py-2 border border-gray-200 rounded-lg text-[var(--text-color-light)] font-semibold"
        >
          {{ tl("log-in-button") }}</RouterLink
        >
        <RouterLink
          to="/register"
          class="hover:-translate-y-0.5 hover:scale-101 hover:shadow-lg duration-150 transition px-4 py-2 bg-[var(--primary-color)] rounded-lg font-semibold text-[var(--text-color-white)]"
          >{{ tl("register-button") }}</RouterLink
        >
      </div>
      ...
      <div class="absolute 2xl:top-0 top-17 right-0">
        <select
          ...
          name="lang"
          id="lang"
          class="bg-[var(--surface-color)] outline-none p-2 m-2 cursor-pointer border border-gray-200 text-[var(--text-color-light)] rounded-lg"
        >
          <option
