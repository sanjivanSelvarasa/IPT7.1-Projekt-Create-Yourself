...
DB_NAME="${DB_NAME:-CreateYourselfDB}"
SA_PASSWORD="${MSSQL_SA_PASSWORD:-${SQLCMDPASSWORD:-}}"
SQLCMD_BIN="/opt/mssql-tools18/bin/sqlcmd"
SCHEMA_FILE="/opt/createyourself/Database/CreateYourselfDB.sql"
...
SQLCMD=("${SQLCMD_BIN}" -S localhost -U sa -P "${SA_PASSWORD}" -C -b)
